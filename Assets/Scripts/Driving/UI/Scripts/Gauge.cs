...
public class Gauge : MonoBehaviour
		...
		if (drawRedZone && redZoneMark != null)
			...
			for (int i=0; i<marks; i++)
				...
				for (int j=1; j<=redZoneMarksPerInterval; j++)
					...
					if (IsSimilarOrGreater(redMarkValue, redZoneStart) &&
						IsSimilarOrSmaller(redMarkValue, redZoneEnd))
						{
		...
		for (int i=0; i<marks; i++)
			...
			if (secondaryMark != null)
				...
				for (int j=1; j<=secondaryMarksPerInterval; j++)
					...
					if (IsSimilarOrSmaller(secondaryAngle, endAngle)
					    && (!drawRedZone || !(IsSimilarOrGreater(markValue, redZoneStart) && IsSimilarOrSmaller(markValue, redZoneEnd)))
						)
						{
	...
	private bool IsSimilarOrGreater(float a, float b)
	{
		return a >= b;
	}
	
	private bool IsSimilarOrSmaller(float a, float b)
	{
		return a <= b;
	}
}
